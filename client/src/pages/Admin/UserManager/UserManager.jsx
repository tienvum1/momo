...
import {
  ...
  Unlock, Lock, Users, UserCheck, ShieldCheck,
  Eye, EyeOff
} from 'lucide-react';
...
const UserManager = () => {
  ...
  const [stats, setStats] = useState({ total: 0, users: 0, admins: 0 });
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      const res = await api.get(`/admin/users`, {
        params: { search, role: roleFilter, status: statusFilter, page: currentPage, limit: itemsPerPage }
      });
      if (res.data.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingUser) {
        ...
        if (!formData.password) return toast.error('Vui lòng nhập mật khẩu cho người dùng mới');
  ...
  const handleToggleStatus = (user) => {
    ...
    setConfirmModal({
      ...
      message: isLocking
        ? `Bạn có chắc chắn muốn khóa tài khoản ${user.email} không?`
              ...
              {users.length === 0 ? (
                <tr>
                  <td colSpan="6" className="empty-row">
                    <div className="empty-state"><AlertCircle size={32} /><p>Không tìm thấy người dùng nào</p></div>
                  </td>
                </tr>
              ) : users.map(user => (
                <tr key={user.id}>
                  <td className="id-col"><span className="id-badge">#{user.id}</span></td>
                  <td>
                  ...
                  <td><div className="date-cell">{new Date(user.created_at).toLocaleDateString('vi-VN')}</div></td>
                  <td>
        ...
        <div className="modal-overlay">
          <div className="confirm-modal-content">
            ...
            <div className="confirm-actions">
              <button className="cancel-btn" onClick={() => setConfirmModal(prev => ({ ...prev, isOpen: false }))}>Hủy bỏ</button>
              <button className={`confirm-btn ${confirmModal.type}`} onClick={confirmModal.onConfirm}>Xác nhận</button>
            </div>
          </div>
        </div>
      )}
